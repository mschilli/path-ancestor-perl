...
Make sure that all paths are given in the same format (either all absolute
or all relative), Path::Ancestor won't do anything fancy to convert them.
Also, the paths given need to be canonical, because Path::Ancestor won't
clean them up or interpret them in terms of a file system -- it is just
a string processor.
